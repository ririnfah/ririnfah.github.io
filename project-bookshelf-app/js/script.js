...
document.addEventListener("DOMContentLoaded", () => {
  ...
  resetBtn.addEventListener("click", () => {
    document.getElementById("pencarian-buku").value = "";    
    searchBook();
  });
//
  const searchForm2 = document.getElementById("formSearch2");
  ...
  resetBtn2.addEventListener("click", () => {
    document.getElementById("pencarian-penulis").value = "";    
    searchWriter();
  });
...
const searchBook = () => {
  const searchInput = document.getElementById("pencarian-buku").value.toLowerCase();
...
const searchWriter = () => {
  const searchInput2 = document.getElementById("pencarian-penulis").value.toLowerCase();
  const writerItems = document.getElementsByClassName("item");

  for (let i = 0; i < writerItems.length; i++) {
    const itemTitle = writerItems[i].querySelector(".item-writer");
    if (itemTitle.textContent.toLowerCase().includes(searchInput2)) {
      writerItems[i].classList.remove("hidden");
    } else {
      writerItems[i].classList.add("hidden");
    }
  }
};